...
module type S = sig
  ...
  (** "connect lines that can be connected modulo epsilon", example:
      ...
      (if [b] abd [b'] are [epsilon]-close to each other.)
      This can result in spectacular simplification, (and faster and sharper PDF
      output) especially for lines generated by {!implicit_curve}. The algorithm
      is O(N) (and quite nice, check it ;) ) *)
