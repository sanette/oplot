...
module type S = sig
  ...
  (** "connect lines that can be connected modulo epsilon", example:

      {[
        Lines [[a;b]; [b';c;d]; [d;e]] -> Lines [a;b;c;d;e]
      ]}

      (if [b] abd [b'] are [epsilon]-close to each other.) This can result in
      spectacular simplification, (and faster and sharper PDF output) especially
      for lines generated by {!implicit_curve}. The algorithm is O(N) (and quite
      nice, check it ;) ) *)
  ...
  module Isocurve : sig
    ...
    type info = {
      ...
      poles : int;
          (** Number of sign changes of [f] that have not been considered as
              zeros, but rather as poles (where the function diverges or is
              highly discontinuous.)*)
      message : Buffer.t;
          (** Reading this string willl give you some debug information. *)
    }
    ...
    (** Use [compute f (p0, p1)] to compute (an approximation) of the level set
        ...
        - [better] is a quick way to obtain a more precise curve without setting
          ...
          etc. Using a negative number will degrade the precision of the curve.
          Of course the default is 0.
