...
module type PATH = sig
  ...
  (** [of_list [a,b,c,...,z]] constructs a path with left end equal to [a]
          and right end equal to [z]. *)
  ...
  (** [concat p1 p2] is the path obtained by appending p2 to the right of p1,
      after removing either the left element of p2 or the right element of
      p1. In principe it should be applied only once you know that the left
      element of p2 is "equal" to the right element of p1. *)
...
module type POINT = sig
  ...
  (** If two elements have the same [hint epsilon] then they must be epsilon close
      to each other: [same epsilon a b = true]. *)
...
module type COMPONENT = functor (P : POINT) ->
  functor (Path : PATH with type point := P.t) -> sig
    ...
    (** Check whether a list of paths is a valid "component", that is: no
        two paths can be concatenated to each other. *)
...
module Point2d = struct
  ...
  type cell = int * int
  let key i j = (i lsl 16) lor j (* TODO use this key for optimizing
                                    hashtables. But we need to make sure i and j
                                    are less than 2^(13 or 14?) to have an
                                    injective key. *)
  let same epsilon z1 z2 =
    abs_float (z1.x -. z2.x) < epsilon && (* comme ça on évite de faire le
                                             deuxième calcul si ce test est déjà
                                             mauvais. **)
    abs_float (z1.y -. z2.y) < epsilon
  let hint epsilon z = Float.to_int (z.x /. epsilon), Float.to_int (z.y /. epsilon)
  let neigh (i0, j0) =
    [i0, j0;
     i0+1,j0+1; i0+1, j0;
     i0+1, j0-1; i0, j0-1; i0-1, j0-1;
     i0-1, j0; i0-1, j0+1; i0, j0+1]
end
...
module List_path (P : POINT) : PATH with type point := P.t = struct
  ...
  let reverse p =
    let content = match p.content with
      | [] -> []
      | c -> List.tl (List.rev (p.left :: c)) in
    { left = p.right; right = p.left; content }

  let concat p1 p2 =
    { left = p1.left; right = p2.right; content = List.append p1.content p2.content }

  (** Measure length *)
  let _length distance p =
    let rec loop d p0 = function
      | [] -> d
      | p1 :: rest -> loop (d +. distance p0 p1) p1 rest in
...
module OList_path (P : POINT) : PATH with type point := P.t = struct
  ...
  let of_pair (a,b) =
    { left = a; content = []; right = b; reverse = false }
...
module Tree_path (P : POINT) : PATH with type point := P.t = struct
  ...
  let reverse p = {
    first = p.last;
    last = p.first;
    tree = (* Reverse p.tree *)match p.tree with
      | Reverse t -> t
      | t -> Reverse t;
  }
  ...
  let of_pair (x,y) = {
    first = x;
    last = y;
    tree = Append(Leaf x, Leaf y)
  }

  let rec to_list_tree acc = function
    | Leaf x -> x :: acc
    | Append (a, b) ->
      to_list_tree (to_list_tree acc b) a
  ...
  and to_list_tree_rev acc = function
    | Leaf x -> x :: acc
    | Append (a, b) ->
      to_list_tree_rev (to_list_tree_rev acc a) b
...
module Component : COMPONENT = functor (P : POINT) ->
  functor (S : PATH with type point := P.t) -> struct
    ...
    (** A [cell] is (the name of) a group of points that are identified (close
     ...
     type edge = {
      ...
      dst : P.cell;
      path: S.t;
      id : int (* this id is used to "pair" an edge from A to B with its reverse
    ...
    let new_id = let id = ref (-1) in fun () -> incr id; !id lsl 1
    ...
    let to_list comp =
      List.map S.to_list comp.paths

    let check_concat epsilon p1 p2 =
      P.same epsilon (S.right p1) (S.left p2)

    let concat epsilon p1 p2 =
      if P.same epsilon (S.right p1) (S.left p2)
      then Some (S.concat p1 p2)
      else None
    ...
    let concat_edges = function
      ...
      | e :: rest ->
        let rec loop acc = function
          | [] -> acc
          | ee :: rrest -> loop (concat_edge ee acc) rrest in
        loop e rest
  ...
  (* Find a suitable cell for the given point *)
    let register_position epsilon pos_tbl p =
      ...
      | Some cell -> cell
      | None ->
        Hashtbl.add pos_tbl cell0 p; cell0
  ...
  let check_max_degree d epsilon paths =
    ...
    let rec loop = function
      | [] -> true
      | path :: rest ->
        let ok1, ok2 = register_degree ~check epsilon pos_tbl deg_tbl path in
  ...
  let () = assert (list_remove_first ( (=) 2)  [1;2;3;4;2;5] = [1; 3; 4; 2; 5])
  ...
  let extract_path adj_tbl cell1 =
    ...
    let adj1 = match Hashtbl.find_opt adj_tbl cell1 with
      | None ->
        Debug.print "Cannot extract a path from the given cell since it is not \
                     in the adj_tbl.";
        assert false
      | Some a -> a in
    loop [] cell1 adj1
  ...
  let make_component epsilon paths =
    ...
    let rec loop found_one paths non_treated = function
      | [] -> if non_treated = [] then paths
        else if found_one then begin (* New pass *)
          loop false paths [] non_treated
        end
        else begin (* Looking for CYCLES*)
      ...
      | cell1 :: rest -> begin
          ...
          | None -> (* pr "  already visited."; *)
            loop found_one paths non_treated rest
          | Some [] -> (* pr "  already taken by another path"; *)
            loop found_one paths non_treated rest
  ...
  let concat_lists epsilon lists =
    List.map S.of_list lists
    |> make_component epsilon
    |> to_list
...
module Test = struct
  ...
  let timeit ?name f n =
    ...
    Debug.print "%stotal time for n=%i     : %fs  (average:%gs)" name n t (t /. float n);
    Debug.print "%stotal CPU time for n=%i : %fs  (average:%gs)\n"
      name n cpu (cpu /. float n)

  let test_pairs ?(pr=true) connect_fn ran () =
    let ps = connect_fn ran in
    let len = List.length ps in
    let n = List.length ran in
    if pr then Debug.print "Initial pairs=%u; Number of paths=%u; number of nodes=%u; \
                            reduction coeff:%.2f" n
        len (List.length (List.flatten ps)) (float (len + n) /. float (2*n))

  let room_size = 500
  let ran_pairs n = List.init n (fun _ -> [Random.int room_size; Random.int room_size])
  ...
  let test1 () =
    ...
    assert (C.concat_pairs 1. pairs |> C.to_list = [[8; 6]; [7; 6; 5; 4; 3]; [1; 2]]);
    ...
    let comps = List.map (fun x -> [x]) (List.rev_append my_comp my_comp2)
      |> List.map (C.make_component 1.) in
  ...
  let test2 () =
    let pairs = [(4,5); (6,7); (6,8); (3,4); (5,6); (1,2)] in
    assert (C2.concat_pairs 1. pairs |> C2.to_list = [[8; 6]; [7; 6; 5; 4; 3]; [1; 2]]);
    let lists = List.map (fun (x,y) -> [x;y]) pairs in
    ...
    let comps = List.map (fun x -> [x]) (List.rev_append my_comp my_comp2)
      |> List.map (C2.make_component 1.) in
  ...
  let test3 () =
    let pairs = [(4,5); (6,7); (6,8); (3,4); (5,6); (1,2)] in
    assert (C3.concat_pairs 1. pairs |> C3.to_list = [[8; 6]; [7; 6; 5; 4; 3]; [1; 2]]);
    let lists = List.map (fun (x,y) -> [x;y]) pairs in
    ...
    let comps = List.map (fun x -> [x]) (List.rev_append my_comp my_comp2)
      |> List.map (C3.make_component 1.) in
  ...
  let test4 () =
    let pairs = [(4,5); (6,7); (6,8); (3,4); (5,6); (1,2)] in
    assert (C4.concat_pairs 1. pairs |> C4.to_list = [[8; 6]; [7; 6; 5; 4; 3]; [1; 2]]);
    let lists = List.map (fun (x,y) -> [x;y]) pairs in
    ...
    let comps = List.map (fun x -> [x]) (List.rev_append my_comp my_comp2)
      |> List.map (C4.make_component 1.) in
