...
let get_dpi () =
  ...
  | Error _ ->
      (* TODO: SDL doc: « This reports the DPI that the hardware reports, and it
       is not always reliable! It is almost always better to use
      ...
      | Some d1, Some d2 ->
          if abs (d1 - d2) > 5 then begin
            Debug.print
              "xrandr and xdpyinfo do not return the same dpi: %u != %u. \
