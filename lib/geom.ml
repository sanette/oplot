...
let in_segment p (p1, p2) =
  let (a, a1, a2) = if abs_float (p2.y -. p1.y) > abs_float (p2.x -. p1.x) then 
    (p.y, p1.y, p2.y) else (p.x, p1.x, p2.x) in
    in_interval a a1 a2
...
let inter_segments (p1, p2) (q1, q2) =
  let (x,y) = try cramer (* intersection des droites directrices *) 
    (p2.y -. p1.y) (p1.x -. p2.x) (det p1.x p2.x p1.y p2.y) 
    (q2.y -. q1.y) (q1.x -. q2.x) (det q1.x q2.x q1.y q2.y) 
  with _ -> (print_endline "Bof"; raise Empty_Intersection) in
  let p = {x=x; y=y} in
    if in_segment p (p1, p2) && in_segment p (q1, q2) then p else
      raise Empty_Intersection;;

(* intersection du segment s avec la boîte rectangulaire donnée par les coins
   diagonaux p1, p2 *)
let inter_box s (p1, p2) =
  let q1, q2, q3, q4 = (p1, {x=p2.x; y=p1.y}, p2, {x=p1.x; y=p2.y}) in
    try inter_segments s (q1, q2) with
      | _ -> try inter_segments s (q2, q3) with
	  | _ -> try inter_segments s (q3, q4) with
	      | _ -> try inter_segments s (q4, q1) with
		  | _ -> raise Empty_Intersection;;
...
let deriv f x = let dx = 0.000001 in (f (x +. dx) -. f (x -. dx)) /. dx /. 2.;;
...
let curve_length fx fy t0 t1 =
  ...
  let rec loop n somme =
    if n=0 then somme
    else let t = t0 +. Random.float (t1 -. t0) in
    let dx = deriv fx t
    and dy = deriv fy t in
      loop (n-1) (somme +. sqrt (dx**2. +. dy**2.)) in
    (loop nt 0.) *. dt;;
...
let q_matrix (q1,q2,q3,q4) = 
  [|[|1. -. 2.*.(q3*.q3+.q4*.q4); 2.*.(q2*.q3 -. q1*.q4); 2.*.(q1*.q3 +. q2*.q4); 0.|]; 
    [|2.*.(q2*.q3 +. q1*.q4); 1. -. 2.*.(q2*.q2+.q4*.q4); 2.*.(q3*.q4 -. q1*.q2); 0.|]; 
    [|2.*.(q2*.q4 -. q1*.q3); 2.*.(q1*.q2 +. q3*.q4); 1. -. 2.*.(q2*.q2+.q3*.q3); 0.|];
    [| 0.; 0.; 0.; 1.|]|]
;;
