...
(*** interactif: renvoie le nombre de millisecondes écoulées depuis la
...
let sdl_init ~show () =
  if List.mem `VIDEO (Sdl.was_init ()) then begin
    ...
    let crucial () =
      ...
      if not show then ignore (Sdlwm.iconify ());
      Sdlttf.init ();
    ...
    (*GlClear.color (float_of_color !default_bg_color);
      GlClear.clear [ `depth ];
      Sdlgl.swap_buffers ();*)
    Sdlevent.disable_events (Sdlevent.all_events_mask lxor
                             (Sdlevent.keydown_mask lor
                              Sdlevent.mousebuttondown_mask lor
...
let gl_init ?(show=true) () = ( match !default_gl with
    | GLUT -> Iglut.init ()
    | SDL -> sdl_init ~show ()
    | GTK -> gtk_init ()
  );
  ...
  begin
    ...
    Gl.disable `polygon_smooth; (* sinon on voit les triangulations *)
    GlMisc.hint `line_smooth `fastest
  end;
  Gl.enable `blend;

  (* Gl.enable `point_smooth; *) (*peut causer d'énormes ralentissements sur certaines implémentations !! *)
  ...
  GlMat.push (); (* on sauve la position initiale *)
  Gl.flush();
...
let feedback_parse_line r n0 nmax = 
  let rec loop n p0 c0 d0 pl depsum nombre =
    ...
    then (pl,depsum,nombre,n)
      ...
      if ((color_of_vertex v1) <> c0) (* change de couleur *)
 || ((point_of_vertex v1) <> p0) (* ligne non continue *)
 || ((depth_of_vertex v1) <> d0) (* change de profondeur  *)
      then (pl,depsum,nombre,n)
 ...
 loop (n+1+2*gl_vertex_size) p2 c0 d0 (p2::pl) (depsum +. d) (nombre + 1) 
  ...
  let (pl,depsum,nombre,n) = loop (n0+1+2*gl_vertex_size) p2 c1 d [p2; point_of_vertex v1] d 2 in
...
let feedback_parse_poly r n0 nmax = 
  let rec loop n nfin pl depsum nombre =
    ...
    let p = point_of_vertex v
    and d = depth_of_vertex v in
      loop (n + gl_vertex_size) nfin (p::pl) (depsum +. d) (nombre + 1) in
  let num = int_of_float (Raw.get_float r ~pos:(n0+1)) in
  let v0 = get_vertex r (n0+2) in
  let c0 = color_of_vertex v0 in
  let (pl,depsum,nombre,n) = loop (n0+2) (n0+1+num*gl_vertex_size) [] 0. 0 in
...
let feedback_parse r nmax =
 ...
 if token = gl_point_token then feedback_parse_point r n nmax
         ...
         token = gl_line_reset_token then feedback_parse_line r n nmax
...
let enter3d ({ Point3.x=x1; y=y1; _ }, {Point3.x=x2; y=y2; _ })  =
  ...
  GlMat.push ();
  GlMat.load_identity ();
  GlMat.ortho ~x:(x1, x2) ~y:(y1, y2) ~z:(-.100., 100.); (* à remplacer par les valeurs zmin zmax, ajustées pour permettre la rotation *)
  GlMat.mode `modelview;
  GlMat.load_identity ();
...
let user_flush = function
  ...
  | GL -> ( match !default_gl with
       | GLUT -> Iglut.swapbuffers ()
       | SDL -> Sdlgl.swap_buffers ()
       | GTK -> () (* ??? *)
   )
  | FIG -> ()
...
let sdl_get_pixel surface x y =
  let p = Sdlvideo.get_pixel surface ~x ~y in
  Sdlvideo.get_RGBA surface p
...
\\begin{preview}\n";
 ...
 shell "dvips -D 600 %s.dvi -o %s.ps" base_name base_name;
  shell "gs -sDEVICE=pngalpha -dTextAlphaBits=4 -r%d -dGraphicsAlphaBits=4 -dSafer -q -dNOPAUSE -sOutputFile=%s.png %s.ps -c quit"
   ...
   (size *  6) base_name base_name;
 let image = Sdlloader.load_image (Printf.sprintf "%s.png" base_name) in
...
let image_of_sdl image =
  ...
  for i = 0 to w - 1 do
    for j = 0 to h - 1 do
      Raw.sets (GlPix.to_raw pixel) ~pos:(4*(j*image_width+i))
 (let ((a,b,c),d) = sdl_get_pixel image i (h -1 - j) in [|a;b;c;d|])
    done
  done;
  pixel,w,h
...
let draw_image ?(mode = (`mode `modulate)) image x0 y0 =
  List.iter (GlTex.parameter ~target:`texture_2d)
    [ `mag_filter `nearest;
      `min_filter `nearest ];
  GlTex.image2d image;  (* utilisation de image comme texture *)
  ...
  let (rx,ry)=(fw /. !fwindow_width , fh /. !fwindow_height ) in (* ratios à adapter à bounding box : draw of pixel !window_width !window_height. 601 au lieu de 600 pour un petit pb d'arrondi (?) que je ne comprends pas trop *)
  GlTex.env mode;
  (* =VERY IMPORTANT: but why any color seem to work ??? *)
  GlDraw.begins `quads;
  GlTex.coord2(0.0, 0.0); GlDraw.vertex2(x0, y0);
  GlTex.coord2(0.0, 1.0); GlDraw.vertex2(x0, y0 +. ry);
  GlTex.coord2(1.0, 1.0); GlDraw.vertex2(x0 +. rx , y0 +. ry);
  GlTex.coord2(1.0, 0.0); GlDraw.vertex2(x0 +. rx , y0);
  GlDraw.ends ();
...
let draw_lines pl ?(dev = !default_device) ?dep view =
  if dev = FIG then  (* latex n'aime pas les dimensions trop grandes*)
    let pls = match view with
      | None -> raise View_expected
...
let move3d m =
  let t0 = match m.init_time with
    | None -> (match m.move with
        | Translate _ -> ()
        | Rotate _ -> ()
...
let rec draw_grid gl ?(dev = !default_device) ?(wire=true) plot_func m (p1,p2) =
  let h = Array.length m - 1
  and w = Array.length m.(0) - 1 in (* w,i=lignes, h,j=colonnes *)
  begin
    let { r=r; g=g; b=b } = !current_color in
    match dev with
    ...
    | GL ->  enter3d (p1,p2);
      begin
        ...
        | None -> (* on stocke les ordres graphiques dans une "display_list" opengl *)
          ...
          if wire then (* rem: pas pour xfig, utiliser des polygones à bord plutôt  ? *)
            begin
...
let draw_segments pl  ?(dev = !default_device) ?dep view = 
              ...
              (Array.of_list 
                 (let rec pair l = match l with
                       ...
                       (int_of_float x0, int_of_float y0,
                        int_of_float x1, int_of_float y1) :: pair ll
                     | _ -> [] in
                  pair ps))
  | GL -> GlDraw.begins `lines;
    List.iter GlDraw.vertex2 ps;
    GlDraw.ends ()
  | FIG ->  let depth = get_depth dep
    ...
    List.iter 
      ...
      (let rec pair l = match l with
          | (x0,y0) :: (x1,y1) :: ll ->
            (int_of_float x0, int_of_float y0, int_of_float x1, int_of_float y1) :: pair ll
          | _ -> [] in
       pair ps)
...
let draw_text ?(dev = !default_device) ?dep view t = 
  ...
  | GL -> let (image,w,h) = match t.pix with
      | Some (pix,w,h) when not !force_refresh -> (pix,w,h)
      (* : on ne calcule l'image qu'une fois ! *)
      | _ -> let pix = (text_image t.text (iscale t.size) t.flag) in
        t.pix <- Some pix; pix in
  ...
  | FIG ->  
    ...
    let fig_align = (match t.align with
        ...
        | LEFT -> 0
        | RIGHT -> 2) in
    Printf.fprintf !xfig_main_channel (*modifier taille et le flag special ?*)
...
let window_flush ?(dev = !default_device) () = match dev with
    X11 -> Graphics.synchronize()
  | GL -> (*Gl.finish () ? *) Sdlgl.swap_buffers()
...
let gl_zoom_in () = mincr zoom3d; mincr zoom3d
...
let sdl_mouse_action mouse = 
  ...
  match but with
  | [Sdlmouse.BUTTON_LEFT] -> gl_mouse_motion x (!window_height - y)
  | _ -> sdl_mouse_close ()
...
let sdl_mouse_init mouse =
  ...
  | Sdlmouse.BUTTON_LEFT ->   
    begin
      ...
      let (x, y)=(mouse.Sdlevent.mbe_x, mouse.Sdlevent.mbe_y) in
      mouse_x :=  x; mouse_y := (!window_height - y)
    end
  | Sdlmouse.BUTTON_WHEELUP -> gl_zoom_in ()
  (* begin
     sdl_mouse_update (Sdlevent.MOUSEBUTTONUP mouse);
     mincr zoom3d; mincr zoom3d
     end *)
  | Sdlmouse.BUTTON_WHEELDOWN -> gl_zoom_out ()
  (* begin
     sdl_mouse_update (Sdlevent.MOUSEBUTTONUP mouse);
...
let sdl_event () =
  (* Sdlevent.wait (); *) (* à mettre si pas d'anim *)
  (* retourne true si on doit quitter (faire une exception ?) *)
  match Sdlevent.poll () with
  | None -> false
  | Some Sdlevent.MOUSEBUTTONDOWN mouse -> sdl_mouse_init mouse; false
  | Some Sdlevent.MOUSEMOTION mouse -> sdl_mouse_action mouse; false
  | Some Sdlevent.MOUSEBUTTONUP _ -> sdl_mouse_close (); false
  | Some Sdlevent.KEYDOWN key -> sdl_key key
  | Some Sdlevent.VIDEORESIZE (w,h) -> (sdl_resize w h); false
  | _ -> false
...
let clear_sheet ?(dev = !default_device) c =  (* à modifier pour n'effacer que la view *)
  match dev with
    X11 -> Graphics.clear_graph (); (* la couleur est ignorée *)
  | GL -> GlClear.color (float_of_color c);
    GlClear.clear [ `color ]
...
let rec graphics_mainloop sh =
  ...
  Graphics.set_color (Graphics.rgb r g b); (* ne pas mettre ici ? *)
  current_color := default_color;
  Graphics.clear_graph (); (* ne pas mettre ici ? *)
...
let xfig_init () =
  ...
  output_string !xfig_head_channel "#FIG 3.2  Produced by oplot.ml, Vu Ngoc San\n";
...
let rec has_latex sh = match sh with
    Sheet [] -> false
...
let write_eps ?(pdf=true) sh =
  ...
  then (shell "%s --input=%s %s" convert latex_header xfig_output_file;
        ...
        if not pdf then begin
          ...
          shell "grep -v \"%%DocumentPaperSizes:\" %s.tmp.ps > %s"
            output output;
          shell "rm %s.tmp.ps" output
        end)
  else shell "fig2dev -L %s -F %s %s" (if pdf then "pdf" else "eps")
...
let disp ?(dev = !default_device) ?(fscreen = false) sh  =
  ...
  | GL -> gl_init ();
    ...
    if wait then (Gl.enable `line_smooth; GlMisc.hint `line_smooth `nicest)
...
let display ?(dev = !default_user_device) ?(fscreen = false) 
  ...
  | IMG_d ->
    (write_bmp (Sheet sh);
     match viewer with
     | Some prog -> shell "%s %s &" prog bmp_output
...
(**************************)
let interrupt () = interrupt_request := true;
