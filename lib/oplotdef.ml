...
let initialize_view3 v = 
  match !current_view3d with
    | None ->  (current_view3d := Some v; v)
    | Some v3 -> v3
 
let reset_view3 () = current_view3d := None

let get_view3 () = match !current_view3d with
  | Some v3 -> v3
...
type motion3d = | Translate of point3
...
type plot_object =
  ...
  | Freeze of int
  | Clear of color
  | Adapt of ((view option) * (plot_object option)) ref * (view option -> plot_object) (* f doit être une fonction qui donne quelque chose même si view option = None. Ca peut servir à initialiser la view avec maxview *)
...
(* animation ? *)
let rec has_anim sh = match sh with
  | Sheet [] -> false
  | User _ -> true
  | Move3d _ -> true
...
(* pauses ? *)
let rec has_pause sh = match sh with
    Sheet [] -> false
...
let text s ?(size=12) ?(align=CENTER) x0 y0 = 
  Text ( { pos = point (x0, y0) ; text = s ; size = size ; 
    align = align ; flag = Normal ; pix = None } )

(*** le même avec le flag Latex ***)
let latex s ?(size=18) ?(align=CENTER) x0 y0 = 
Text ( { pos = point (x0, y0) ; text = s ; size = size ; 
    align = align ; flag = Latex ; pix = None } )
...
let lines_crop lines v =
  let rec loop l1 prev acc lacc = 
    match l1 with
      | [] -> if acc = [] then lacc else acc::lacc
      | p::rest -> begin match (inside p v, acc) with
   | (true, []) (* on rentre *) -> let new_acc = begin match prev with
       | None -> [p]
       | Some p' -> let p'' = try intersect p p' v with _ -> p' in [p;p'']
     end in
   ...
   | (false, _) (* on sort *) -> begin match prev with
       | Some p' -> let p'' = try intersect p p' v with _ -> p' in 
    loop rest (Some p) [] ((p''::acc)::lacc)
       | None -> failwith "point expected in line_crop!" end
 end in
    loop lines None [] []

(* crée un objet Adapt pour un graphe de fonction *)
let adapt_plot f ?pas x0 x1 =
  let list = point_list f ?pas x0 x1 () in (* adapter aussi le pas ? *)
    Adapt (ref (None, None), 
...
let parametric_list fx fy t0 t1 pas =
  let rec loop list t =
    if abs_float (t +. pas -. t0) > abs_float (t1 -. t0) 
    then ({ Point2.x=(fx t1) ; y=(fy t1) } :: list)
    else loop ({ Point2.x=(fx t) ; y=(fy t) } :: list) (t +. pas) in
    loop [] t0

let parametric_plot fx fy ?pas ?(adapt=true) t0 t1 = 
  let p = match pas with
      None -> (t1 -. t0) /. 
              (min ((Geom.curve_length fx fy t0 t1) *. !fwindow_width) (float maxpoints))
    | Some pp -> pp in
  let list = parametric_list fx fy t0 t1 p in
  if adapt then Adapt (ref (None, None), 
                       fun view -> match view with
                         | None -> lines_of_list list
                         | Some v -> Lines (lines_crop list v))
...
let list_plot l x0 x1 = 
  ...
  let rec loop list plist i =
    match list with
      | [] -> plist
      | y::ll -> loop ll ({Point2.x = x0 +. float (i+1) *. dx; y = y} :: 
       {Point2.x = x0 +. float i *. dx; y = y} ::  plist) (i+1)
  in Lines [(loop l [] 0)]
...
let rec loop xmax xmin ymax ymin l =
  match l with
    | (x,y)::ll -> loop (max xmax x) (min xmin x) (max ymax y) (min ymin y) ll
    | [] -> view xmin ymin xmax ymax in
  match l with
...
let label_dot_plot ?(dot = just_a_dot) ?(offset=0.005) ?view l =
  ...
  let v = match view with
    | Some vv -> vv
    | None -> let coord_list = List.rev_map (fun (x,y,_) -> (x,y)) l in
      find_view coord_list in
  (v :: (loop l []))
...
let matrix_plot_f f ?(width = !window_width/4) ?(height = !window_height/4) x0 y0 x1 y1 =
  ...
  for i=0 to (height - 1) do
    for j=0 to (width - 1) do
      let z =  f (x0 +. (float j +. 0.5) *. dx) 
          (y0 +. (float i +. 0.5) *. dy) in
      fm.(i).(j) <- z;
...
(* les données "normales" sont les indices 1 à width+1, 1 à height+1 *)
 ...
 for j=(-1) to width+1 do
   ...
   and z =  fz (u0 +. (float j) *. du) (v0 +. (float i) *. dv) in
    ...
    Surf3d ((mx, my, mz,
      (point3 (!xmin, !ymin, !zmin), point3 (!xmax, !ymax, !zmax)), wire), ref None)

(*** interactif: crée un objet Axis ***)
let axis x0 y0 = Axis ( { center = point (x0,y0) ; view = None ; ticks = None ; 
     window_size = (!window_width , !window_height)} )

(*** interactif: crée un objet Color (pas très utile, on peut le faire
     à la main ! ***)
let color r g b = Color { r=r ; g=g ; b=b }

let pause t = Pause t

let freeze t = Freeze t

let translate v t = Move3d {move = (Translate v); time = {min=0.; max=t};
                            init_time = None}
...
let zoom z z0 t0 t1 = Move3d {move = (Zoom (z, z0)); time = {min=t0; max=t1};
                              init_time = None}
