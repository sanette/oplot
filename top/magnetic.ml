...
let a1 x y = (-. y) *. (1. +. x *. x /. 4. +. y *. y /. 4. +. x *. x *. x /. 3.);;
...
(* dA_1/dq_2 *)
let a12 [|x; y|] = (-. 1.) -. x *. x /. 4. -. 3. *. y *. y /. 4. -. x *. x *. x /. 3.;;
(* dA_2/dq_1 *)
...
let anm = anim_ode ~time_pos:{x=1.; y=1.} ~show_head:true ~speed:10. 
  magnetic (init epsilon x0 y0) 0. 500.;;

let anm_slow = anim_ode ~time_pos:{x=1.; y=1.} ~show_head:true ~speed:0.5 
  magnetic (init epsilon x0 y0) 0. 4.;;

display [vw; Color red; cnt; cnt_text; Color blue; anm_slow; r_text; Color black; axis 0. 0.; Pause 0];;

display [vw; Color red; cnt; cnt_text; Color blue; anm; Color black; axis 0. 0.];;
